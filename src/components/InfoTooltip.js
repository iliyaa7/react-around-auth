    ...
    <div className={`popup ${props.isOpen && "popup_opened"}`} id="picture">
      <div className="popup__grid">
        ...
        <div className="popup__holder popup__holder_type_tool-tip">
          <img src={props.isSuccessfull ? successPath : failedPath} alt="Succes icon" className="popup__image popup__image_type_tool-tip" id="popup__image"/>
          <p className="popup__caption popup__caption_type_tool-tip">{props.isSuccessfull ? "Success! You have now been registered." : "Oops, something went wrong! Please try again."}</p>
        </div>
      </div>
    </div>
  );
